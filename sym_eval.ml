...
= fun senv pi e ->
  ...
  | ADD (e1, e2) ->
    ...
    List.map (fun ((p1, v1), (p2, v2)) -> (PPop ((&&), p1, p2), SAop ((+), v1, v2))) car
  | SUB (e1, e2) ->
    let car = cartesian (sym_eval senv pi e1) (sym_eval senv pi e2) in
    List.map (fun ((p1, v1), (p2, v2)) -> (PPop ((&&), p1, p2), SAop ((-), v1, v2))) car
  | MUL (e1, e2) ->
  ...
  | MOD (e1, e2) ->
    let car = cartesian (sym_eval senv pi e1) (sym_eval senv pi e2) in
    List.map (fun ((p1, v1), (p2, v2)) -> (PPop ((&&), p1, p2), SAop ((mod), v1, v2))) car
  | MINUS e -> List.map (fun (p, v) -> (p, SAop ((-), SInt 0, v))) (sym_eval senv pi e)
  | NOT e -> List.map (fun (p, v) -> (p, SBop ((fun a b -> not a)))) sym_eval senv pi e
  | OR (e1, e2) ->
    let car = cartesian (sym_eval senv pi e1) (sym_eval senv pi e2) in
    List.map (fun ((p1, v1), (p2, v2)) -> (PPop ((&&), p1, p2), SBop ((||), v1, v2))) car
  | AND (e1, e2) ->
    let car = cartesian (sym_eval senv pi e1) (sym_eval senv pi e2) in
    List.map (fun ((p1, v1), (p2, v2)) -> (PPop ((&&), p1, p2), SBop ((&&), v1, v2))) car
  | LESS (e1, e2) ->
