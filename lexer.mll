...
let create_token lexbuf =
  ...
  match lookup str reserved_words with
  | None -> LID str
  | Some t -> t

let create_symbol lexbuf =
  let str = lexeme lexbuf in
  match lookup str symbols with
  | None -> raise @@ Lexer_error ("Unexpected token: " ^ str)
  | Some t -> t

let create_external lexbuf =
  let str = lexeme lexbuf in
  match lookup str reserved_words in
  | None -> raise @@ Lexer_error ("Unexpected token: " ^ str)
  | Some t -> t
}
...
rule token =  parse
  ...
  | _ as c { raise @@ Lexer_error ("Unexpected charactor: " ^ Char.escaped c) }
...
and create_string buf = parse
 ...
 | _ as c { Buffer.add_char buf c; create_string buf lexbuf }
